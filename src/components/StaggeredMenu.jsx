...
export const StaggeredMenu = ({
  position = 'right',
  colors = ['#0f172a', '#3b82f6', '#1e293b', '#334155'],
  items = [],
  socialItems = [],
  displaySocials = true,
  ...
  const animateIcon = useCallback(opening => {
    ...
    gsap.to(iconRef.current, { rotate: opening ? 225 : 0, duration: opening ? 0.8 : 0.35, ease: 'power4.out', overwrite: 'auto' });
  }, []);

  const animateColor = useCallback(opening => {
    if (!toggleBtnRef.current || !changeMenuColorOnOpen) return;
    gsap.to(toggleBtnRef.current, { color: opening ? openMenuButtonColor : menuButtonColor, delay: 0.18, duration: 0.3, ease: 'power2.out' });
  ...
  const animateText = useCallback(opening => {
    ...
    gsap.to(textInnerRef.current, { yPercent: -75, duration: 0.8, ease: 'power4.out' });
  }, []);
  ...
  return (
    <div className={`staggered-menu-wrapper ${className || ''}`} data-position={position} data-open={open || undefined}>
          ...
          <ul className="sm-panel-list" data-numbering={displayItemNumbering || undefined}>
            {items.map((it, idx) => (
              <li className="sm-panel-itemWrap" key={idx}>
                <button className="sm-panel-item" onClick={() => { if (it.onClick) it.onClick(); closeMenu(); }}>
